...
const KPI = ({ statistics, selectedDepartment = "dept_cs" }) => {
    ...
    try {
      ...
      const researchersPromises = facultySnapshot.docs.map(async (facultyDoc) => {
        const faculty = {
          ...
          departmentName: DEPARTMENT_NAMES[selectedDepartment] || selectedDepartment,
        ...
        const publicationsRef = collection(
          db,
          `${basePath}/${selectedDepartment}/faculty_members/${facultyDoc.id}/publications`
        );
      ...
      const fetchedResearchers = await Promise.all(researchersPromises);
  ...
  useEffect(() => {
    fetchResearchers();
  }, [selectedYear, selectedDepartment, fetchResearchers]);
  ...
  const kpiData = useMemo(() => [
    ...
    },
    {
      ...
      description: `Average number of citations per publication across all years`
    }
  ...
  const getFilteredResearchersCount = () => {
    if (viewingKpi === "KPI-I-13") {
      ...
      return filteredResearchers.length;
    }
  };
          ...
          <div className="flex flex-col">
            <div className="flex flex-wrap justify-between items-center mb-2">
              <Typography variant="h6" className="mr-2">
                {viewingKpi === "KPI-I-13" ? "Faculty with Publications" : "Faculty Members"}
              </Typography>
        ...
        <DialogContent dividers>
          {loading ? (
            ...
            <List sx={{ width: '100%', bgcolor: 'background.paper' }}>
              {(viewingKpi === "KPI-I-13" ? 
                filteredResearchers.filter(r => r.hasPublishedInSelectedYear) : 
                filteredResearchers
              ).map((researcher) => (
                <ListItem
                  key={researcher.id}
              ...
              {!loading && (viewingKpi === "KPI-I-13" ? 
                filteredResearchers.filter(r => r.hasPublishedInSelectedYear).length === 0 : 
